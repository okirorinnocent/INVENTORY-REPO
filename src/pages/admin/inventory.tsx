...
export function AdminInventory() {
  ...
  const handleSave = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const url = isEditing ? `/api/products/${isEditing._id}` : '/api/products';
        ...
        <table className="w-full text-left border-collapse">
          ...
          <tbody>
            {filteredProducts.map(product => (
              <tr key={product._id} className="border-b border-neutral-100 hover:bg-neutral-50/50 transition-colors">
                ...
                <td className="p-4">
                  <span className={`px-3 py-1 rounded-full text-xs font-bold ${product.stock > 20 ? 'bg-emerald-100 text-emerald-700' :
